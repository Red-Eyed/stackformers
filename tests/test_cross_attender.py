...
def test_cross_attender_output_shape(
    ...
    x_context: tuple[torch.Tensor, torch.Tensor],
) -> None:
    x, context = x_context
    out = cross_attender(x, context)
    assert out.shape == (B, N, D)


def test_cross_attender_no_ctx_seq_info(
    cross_attender: CrossAttender,
    x_context: tuple[torch.Tensor, torch.Tensor],
) -> None:
    x, context = x_context
    out = cross_attender(x, context, ctx_seq_info=None)
    assert out.shape == (B, N, D)
...
def test_cross_attender_with_ctx_padding(
    ...
    mask[1, 8:] = False
    out = cross_attender(x, context, ctx_seq_info=PaddedSequence(mask=mask))
    assert out.shape == (B, N, D)
