...
| `factory.py` | `build_ff(config: FeedForwardConfig) -> FeedForward` |
...
3. Add a `case NewFFConfig()` branch in `factory.py::build_ff` (currently a direct call — convert to `match` when a second variant exists).
